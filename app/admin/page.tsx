    ...
    <div style={s.page}>
      ...
      <header style={s.navbar}>
        <div style={s.navInner}>
          <span style={s.navLogo}>💰 FinanceApp</span>
          <nav style={s.nav}>
            <a href="/dashboard" style={s.navLink}>Dashboard</a>
            <a href="/admin" style={s.navLinkActive}>Administração</a>
          </nav>
          <div style={s.navUser}>
            <span style={s.navUserName}>{user?.name}</span>
            <button style={s.logoutBtn} onClick={() => { localStorage.clear(); router.push("/login"); }}>Sair</button>
          </div>
        </div>
      </header>
